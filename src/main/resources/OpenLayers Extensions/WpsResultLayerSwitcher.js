...
OpenLayers.Control.WpsResultLayerSwitcher = 
  ...
  // DOM Elements
        ...
        for(var i=0, len=layers.length; i<len; i++) {
            ...
            if (layer.displayInLayerSwitcher) {
                ...
                if(layer.CLASS_NAME == "WOC.VectorStoringGML") {
                    ...
                    showGMLButton.type = 'image';
                    showGMLButton.src = 'img/icon_gml.png';
                    ...
                    deleteLayerButton.type = 'image';
                    deleteLayerButton.src = 'img/cross.png';
                    ...
                    // show KML Button
					var showKMLButton = document.createElement('input');
                    showKMLButton.type = 'image';
                    showKMLButton.src = 'img/icon_kml.png';
                    showKMLButton.layer = layer;
                    OpenLayers.Event.observe(showKMLButton, "click",
                                    OpenLayers.Function.bindAsEventListener(
                                                    this.showKML, showKMLButton));
                    ...
                    OpenLayers.Event.observe(deleteLayerButton, "click",
                                    OpenLayers.Function.bindAsEventListener(
                                                    this.deleteKML, deleteLayerButton, {'layersw':this}));
                ...
                var groupDiv = (baseLayer) ? this.baseLayersDiv
 				...
 				if(!baseLayer){
					if(layer.CLASS_NAME == "WOC.VectorStoringGML"){
						...
						groupDiv.appendChild(colorPickerInput);					// adding the color picker input
						groupDiv.appendChild(labelSpan);
						groupDiv.appendChild(showKMLButton);					// adding the showKML button
						groupDiv.appendChild(deleteLayerButton);				// adding the delete layer button
						groupDiv.appendChild(br);							
					}
					else {
    ...
    showKML: function(event) {
        ...
        var d = newwin.document;
        // Because the name has whitespace it has to be set here!
        newwin.name = 'KML data';
        d.open("text/html","replace");
        newwin.document.write("<html><head>" +
                        "<title>KML data</title>" +
                        "</head><body><pre>" +
                        this.layer.getKML().replace(/</g, "&lt;").replace(/>/g, "&gt;") +
                        "</pre></body></html>");
        newwin.document.close();
        if (window.focus) {
                newwin.focus();
	...
	// RR: button DEL for deleting KML Output layers
	deleteKML: function(event) {
	         var layers = this.layer.map.layers;
